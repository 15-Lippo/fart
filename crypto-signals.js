...
// Versione 2.1.0 - Aggiornato per utilizzare CoinMarketCap invece di CoinGecko

// ---- CONFIGURAZIONE API ----
const COINMARKETCAP_API_KEY = 'b54bcf4d-1bca-4e8e-9a24-22ff2c3d462c'; // API key gratuita
...
async function checkApiStatus() {
    try {
        ...
        const url = USE_PROXY 
            ? `${PROXY_URL}${COINMARKETCAP_API_URL}/cryptocurrency/listings/latest?limit=10`
            : `${COINMARKETCAP_API_URL}/cryptocurrency/listings/latest?limit=10`;
        ...
        apiAvailable = true;
        return data && data.status && data.status.error_code === 0;
    } catch (error) {
...
export async function getTopCryptos() {
    ...
    try {
        ...
        const url = USE_PROXY 
            ? `${PROXY_URL}${COINMARKETCAP_API_URL}/cryptocurrency/listings/latest?limit=20`
            : `${COINMARKETCAP_API_URL}/cryptocurrency/listings/latest?limit=20`;
...
export async function getCryptoHistoricalData(cryptoId, days = 30) {
    ...
    try {
        ...
        // Converti l'ID della cripto in formato CMC
        const cryptoInfo = await getCryptoInfo(cryptoId);
        if (!cryptoInfo) {
            return getFallbackHistoricalData(cryptoId);
        }
        
        const cmcId = cryptoInfo.id;
        const timeEnd = Math.floor(Date.now() / 1000);
        const timeStart = timeEnd - (days * 86400); // days in secondi
        
        const url = USE_PROXY 
            ? `${PROXY_URL}${COINMARKETCAP_API_URL}/cryptocurrency/quotes/historical?id=${cmcId}&time_start=${timeStart}&time_end=${timeEnd}&interval=daily`
            : `${COINMARKETCAP_API_URL}/cryptocurrency/quotes/historical?id=${cmcId}&time_start=${timeStart}&time_end=${timeEnd}&interval=daily`;
        ...
        const data = await response.json();
        const quotes = data.data[cmcId].quotes;
...
async function getCryptoInfo(cryptoId) {
    ...
    try {
        const url = USE_PROXY 
            ? `${PROXY_URL}${COINMARKETCAP_API_URL}/cryptocurrency/info?slug=${cryptoId}`
            : `${COINMARKETCAP_API_URL}/cryptocurrency/info?slug=${cryptoId}`;
        ...
        // Estrai il primo elemento dall'oggetto data
...
export async function getCryptoSignals() {
    ...
    try {
        if (!await checkApiStatus()) {
            console.warn('API CoinMarketCap non disponibile, utilizzo segnali di fallback');
