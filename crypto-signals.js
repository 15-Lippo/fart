...
// Versione 2.3.0 - Aggiornato per utilizzare CoinPaprika

// ---- CONFIGURAZIONE API ----
const COINPAPRIKA_API_URL = 'https://api-pro.coinpaprika.com/v1';
...
async function checkApiStatus() {
    try {
        ...
        const url = USE_PROXY 
            ? `${PROXY_URL}${COINPAPRIKA_API_URL}/coins`
            : `${COINPAPRIKA_API_URL}/coins`;
...
export async function getTopCryptos() {
    try {
        ...
        // Ordina per market cap e prendi le top 20
        const topCryptos = data
            ...
            .map(crypto => ({
                ...
                name: crypto.name,
                currentPrice: crypto.quotes.USD.price,
                priceChangePercentage24h: crypto.quotes.USD.percent_change_24h,
...
export async function getCryptoHistoricalData(cryptoId, days = 30) {
    try {
        ...
        const url = USE_PROXY 
            ? `${PROXY_URL}${COINPAPRIKA_API_URL}/coins/${cryptoId}/ohlcv/historical?start=${new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString()}`
            : `${COINPAPRIKA_API_URL}/coins/${cryptoId}/ohlcv/historical?start=${new Date(Date.now() - days * 24 * 60 * 60 * 1000).toISOString()}`;
        ...
        const labels = data.map(d => {
            const date = new Date(d.time_open);
...
async function getCryptoInfo(cryptoId) {
    try {
        const url = USE_PROXY 
            ? `${PROXY_URL}${COINPAPRIKA_API_URL}/coins/${cryptoId}`
            : `${COINPAPRIKA_API_URL}/coins/${cryptoId}`;
...
export async function getCryptoSignals() {
    try {
        ...
        if (!isApiAvailable) {
            console.warn('API CoinPaprika non disponibile, utilizzo segnali di fallback');
