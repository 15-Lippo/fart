// Costanti per limitare errori
const DEFAULT_TIMEOUT = 10000; // 10 secondi
const API_RATE_LIMIT_WAIT = 60000; // 60 secondi - CoinGecko ha un limite di 10-30 chiamate al minuto
const CACHE_EXPIRY = 60000 * 5; // 5 minuti di cache
const FREE_API_KEY = "CG-kXyY2z68x6g7qXAx5VnJe6aw";

// Flag per tracciare errori 429 (limite rate)
let hasRateLimitError = false;
let lastRateLimitTime = 0;

// Tempo di attesa minimo tra le chiamate API per evitare rate limiting
let lastApiCallTime = 0;
const API_CALL_SPACING = 1500; // 1.5 secondi tra le chiamate

// Funzione per controllare lo stato dell'API prima di fare richieste
async function checkApiStatus() {
    // Se abbiamo già ricevuto un rate limit, aspetta almeno 1 minuto
    if (hasRateLimitError) {
        const timeSinceLimit = Date.now() - lastRateLimitTime;
        if (timeSinceLimit < API_RATE_LIMIT_WAIT) {
            console.warn(`API in pausa a causa di rate limit. Attesa rimanente: ${Math.ceil((API_RATE_LIMIT_WAIT - timeSinceLimit)/1000)}s`);
            return false;
        }
        hasRateLimitError = false;
    }
    
    // Rispetta l'intervallo minimo tra le chiamate
    const timeSinceLastCall = Date.now() - lastApiCallTime;
    if (timeSinceLastCall < API_CALL_SPACING) {
        await new Promise(resolve => setTimeout(resolve, API_CALL_SPACING - timeSinceLastCall));
    }
    
    try {
        console.log("Verifica stato API CoinGecko...");
        ...
        const response = await fetch('https://api.coingecko.com/api/v3/ping', {
            ...
            headers: {
                ...
                'x-cg-api-key': FREE_API_KEY // Usa l'API key gratuita
            }
        });
        ...
        if (response.status === 429) {
            console.error("API CoinGecko ha restituito 429 - Rate limit superato");
            hasRateLimitError = true;
            lastRateLimitTime = Date.now();
            return false;
        }
        
        // Se la risposta è ok, possiamo procedere
        if (response.ok) {
            console.log("API CoinGecko funzionante");
            return true;
        }
        ...
        console.error("Errore durante il controllo dell'API:", error);
        return false;
    }
}
...
export async function getCryptoSignals() {
    try {
        ...
        // Prima controlla se l'API è disponibile
        const apiAvailable = await checkApiStatus();
        if (!apiAvailable) {
            debugSignalsProcess('API CoinGecko non disponibile, utilizzo segnali hardcoded');
            return getHardcodedSignals();
        }
        
        // Imposta timeout più breve per evitare lunghe attese in caso di errore
        const controller = new AbortController();
        const timeoutId = setTimeout(() => {
            controller.abort();
            debugSignalsProcess('Timeout della richiesta API (15s)');
        }, DEFAULT_TIMEOUT);
        
        console.log('Richiesta segnali in corso...');
        let response;
        try {
            debugSignalsProcess('Avvio fetch API CoinGecko');
            
            // Prova prima una richiesta con meno dati per avere una risposta più veloce
            response = await fetch('https://api.coingecko.com/api/v3/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=50&page=1&sparkline=false', {
                signal: controller.signal,
                headers: {
                    'Accept': 'application/json',
                    'Cache-Control': 'no-cache',
                    'x-cg-api-key': FREE_API_KEY
                }
            });
            ...
            // Se la risposta è un errore 429 (Too Many Requests), ritorna subito i segnali hardcoded
            if (response.status === 429) {
                clearTimeout(timeoutId);
                hasRateLimitError = true;
                lastRateLimitTime = Date.now();
                debugSignalsProcess('Limite API raggiunto (429 Too Many Requests), utilizzo segnali hardcoded');
                return getHardcodedSignals();
            }
        } catch (fetchError) {
            clearTimeout(timeoutId);
            ...
            debugSignalsProcess('Ritorno segnali hardcoded a causa di errore di rete');
            return getHardcodedSignals();
        }
        ...
        if (!response.ok) {
            ...
            debugSignalsProcess('Ritorno segnali hardcoded a causa di risposta API non valida');
            return getHardcodedSignals();
        }
        
        let cryptos;
        try {
            debugSignalsProcess('Parsing JSON della risposta API');
            ...
            debugSignalsProcess('Ritorno segnali hardcoded a causa di errore nel parsing JSON');
            return getHardcodedSignals();
        }
        ...
        if (!cryptos || !Array.isArray(cryptos) || cryptos.length === 0) {
            ...
            debugSignalsProcess('Ritorno segnali hardcoded a causa di risposta API vuota');
            return getHardcodedSignals();
        }
...
function getHardcodedSignals() {
    console.log("Restituzione dei segnali predefiniti aggiornati");
    return [
        {
            id: "bitcoin",
            pair: "BTC/USDT",
        ...
        },
        {
            ...
            stopLoss: "145.0000",
            support: ["145.0000", "140.0000", "130.0000"],
            resistance: ["165.0000", "170.0000", "180.0000"],
            potentialGain: "9.68",
            riskReward: "1:1.50",
            confidence: 65,
            priceChange24h: "3.20",
        ...
        },
        {
            ...
            stopLoss: "530.0000",
            support: ["530.0000", "520.0000", "500.0000"],
            resistance: ["570.0000", "600.0000", "620.0000"],
            potentialGain: "8.11",
            riskReward: "1:1.80",
            confidence: 62,
            priceChange24h: "1.90",
        ...
        },
        {
            ...
            targetPrice: "0.5800",
            stopLoss: "0.4800",
            support: ["0.4800", "0.4600", "0.4500"],
            resistance: ["0.5500", "0.5800", "0.6000"],
            potentialGain: "12.62",
            riskReward: "1:1.85",
            confidence: 60,
            priceChange24h: "2.10",
...
export async function getCryptoFullHistoricalData(id, days = 30, signal) {
    try {
        // Verifica prima se l'API è funzionante
        const apiAvailable = await checkApiStatus();
        if (!apiAvailable) {
            console.warn("API CoinGecko non disponibile, utilizzo dati storici predefiniti");
            return null;
        }
        ...
        const combinedSignal = signal ? signal : new AbortController().signal;
        ...
        // Controlla per rate limit
        if (response.status === 429) {
            hasRateLimitError = true;
            lastRateLimitTime = Date.now();
            console.warn('Rate limit API raggiunto per dati completi');
            return null;
        }
...
export async function getCryptoHistoricalData(symbol, days = 30) {
    try {
        ...
        if (response.status === 429) {
            hasRateLimitError = true;
            lastRateLimitTime = Date.now();
            console.warn('Rate limit API raggiunto. Utilizzo dati predefiniti');
            return getDefaultHistoricalData(symbol);
        }
