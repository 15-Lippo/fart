...
// Versione 2.4.0 - Aggiornato per utilizzare Binance API
...
async function checkApiStatus() {
    try {
        ...
        const url = USE_PROXY 
            ? `${PROXY_URL}${BINANCE_API_URL}/time`
            : `${BINANCE_API_URL}/time`;
...
export async function getTopCryptos() {
    try {
        ...
        const url = USE_PROXY 
            ? `${PROXY_URL}${BINANCE_API_URL}/ticker/24hr`
        ...
        // Filtra solo le coppie USDT e ordina per volume
        const topCryptos = data
            ...
            .map(crypto => ({
                ...
                priceChangePercentage24h: parseFloat(crypto.priceChangePercent),
                marketCap: parseFloat(crypto.volume) * parseFloat(crypto.lastPrice)
            }));
...
export async function getCryptoHistoricalData(cryptoId, days = 30) {
    try {
        ...
        const url = USE_PROXY 
            ? `${PROXY_URL}${BINANCE_API_URL}/klines?symbol=${cryptoId}USDT&interval=1d&startTime=${startTime}&endTime=${endTime}`
            : `${BINANCE_API_URL}/klines?symbol=${cryptoId}USDT&interval=1d&startTime=${startTime}&endTime=${endTime}`;
        ...
        const data = await response.json();
        const prices = data.map(d => parseFloat(d[4])); // Prezzi di chiusura
...
async function getCryptoInfo(cryptoId) {
    try {
        const url = USE_PROXY 
            ? `${PROXY_URL}${BINANCE_API_URL}/ticker/24hr?symbol=${cryptoId}USDT`
            : `${BINANCE_API_URL}/ticker/24hr?symbol=${cryptoId}USDT`;
...
export async function getCryptoSignals() {
    try {
        ...
        if (!isApiAvailable) {
            console.warn('API Binance non disponibile, utilizzo segnali di fallback');
