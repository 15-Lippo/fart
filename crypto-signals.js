...
// Versione 2.5.1 - Risoluzione problemi di caricamento

// ---- CONFIGURAZIONE API ----
const COINGECKO_API_URL = 'https://api.coingecko.com/api/v3';
const USE_PROXY = true; // Usa proxy per evitare CORS
const PROXY_URL = 'https://cors-anywhere.herokuapp.com/'; // Proxy più affidabile
...
// Controlla lo stato dell'API - Semplificato per evitare problemi
async function checkApiStatus() {
    try {
        ...
        // Test semplificato - non facciamo una richiesta effettiva
        // Assumiamo che l'API sia disponibile a meno che non sia stata recentemente
        // disabilitata a causa di rate limit
        console.log('Controllo disponibilità API...');
        return apiAvailable;
    } catch (error) {
...
export async function getTopCryptos() {
    try {
        if (!await checkApiStatus()) {
            console.warn('API non disponibile, utilizzo dati di fallback');
            return getFallbackTopCryptos();
        }
        
        // URL diretto (senza parametri codificati)
        const url = USE_PROXY 
            ? `${PROXY_URL}${COINGECKO_API_URL}/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=20&sparkline=false&price_change_percentage=24h`
            : `${COINGECKO_API_URL}/coins/markets?vs_currency=usd&order=market_cap_desc&per_page=20&sparkline=false&price_change_percentage=24h`;
        
        console.log('Richiesta URL:', url); // Debug log
        ...
        console.log('Dati ricevuti, lunghezza:', data.length); // Debug log
        
        if (!Array.isArray(data)) {
            console.error('Formato dati non valido, utilizzo fallback');
            return getFallbackTopCryptos();
        }
        ...
        console.error('Errore dettagliato nel recupero delle top cryptos:', error);
        return getFallbackTopCryptos();
    }
}
...
export async function getCryptoSignals() {
    try {
        console.log('Inizio recupero segnali...'); // Debug log
        
        // Utilizziamo direttamente i dati di fallback se ci sono stati errori precedenti
        if (!await checkApiStatus()) {
        ...
        console.log('Recupero top cryptos...');
        ...
        console.log('Top cryptos recuperate:', topCryptos.length); // Debug log
        
        // Per essere sicuri di avere sempre dei segnali, utilizziamo i dati di fallback se necessario
        if (topCryptos[0].id === "1") { // Questo indica che stiamo usando i dati di fallback
            console.log('Utilizzando segnali di fallback basati su dati di fallback');
            return getFallbackSignals();
        }
        ...
        for (const crypto of cryptosToProcess) {
            try {
                ...
                signals.push(signal);
                console.log(`Segnale generato per ${crypto.name}`); // Debug log
            } catch (error) {
                console.error(`Errore nella generazione del segnale per ${crypto.name}:`, error);
            }
            
            // Piccola pausa tra le generazioni per evitare problemi
            await sleep(200);
        }
